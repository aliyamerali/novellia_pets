...
RSpec.describe 'New allergy_record page' do
  before :each do
    ...
    visit "user/system/allergy_reactions/new?allergy_record_id=#{@allergy_record.id}"
  end
  ...
  it 'reloads reaction create page on submit' do
    ...
    expect(page).to have_current_path("/user/system/allergy_reactions/new?allergy_record_id=#{@allergy_record.id}")
